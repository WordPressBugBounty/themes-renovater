...
use Renovater\Block_Patterns;
use Renovater\Block_Styles;
use Renovater\Upgrader;
...
class Init {
	...
	public function notice_install_plugin() {
		...
		$button_text = __( 'Install Required Plugins', 'renovater' );
		$button_link = wp_nonce_url( self_admin_url( 'themes.php?page=renovater-dashboard' ), 'install-plugin_gutenverse' );
		?>
		<style>
			.install-gutenverse-plugin-notice {
				...
				background: url( <?php echo esc_url( RENOVATER_URI . '/assets/img/background-banner.png' ); ?> );
		...
		jQuery( function( $ ) {
			$( 'div.notice.install-gutenverse-plugin-notice' ).on( 'click', 'button.notice-dismiss', function( event ) {
				...
				$.post( ajaxurl, {
					action: 'renovater_set_admin_notice_viewed',
					nonce: '<?php echo esc_html( wp_create_nonce( 'renovater_admin_notice' ) ); ?>',
				} );
			} );
		...
		<div class="notice is-dismissible install-gutenverse-plugin-notice">
			...
			<div class="gutenverse-notice-content">
					<div class="gutenverse-notice-text">
						<h3><?php esc_html_e( 'Take Your Website To New Height with', 'renovater' ); ?> <span>Gutenverse!</span></h3> 
						<p><?php esc_html_e( 'Renovater theme work best with Gutenverse plugin. By installing Gutenverse plugin you may access Renovater templates built with Gutenverse and get access to more than 40 free blocks, hundred free Layout and Section.', 'renovater' ); ?></p>
					...
					<div class="gutenverse-notice-image">
						<img src="<?php echo esc_url( RENOVATER_URI . '/assets/img/banner-install-gutenverse-2.png' ); ?>"/>
					</div>
				</div>
			</div>
		</div>
	...
	public function admin_menu() {
		add_theme_page(
			...
			'manage_options',
			'renovater-dashboard',
			array( $this, 'load_renovater_dashboard' ),
			1
		);
	}

	/**
	 * Renovater Template page
	 */
	public function load_renovater_dashboard() {
		?>
			<?php if ( defined( 'GUTENVERSE_VERSION' ) && version_compare( GUTENVERSE_VERSION, '1.1.1', '<=' ) ) { ?>
			<div class="notice is-dismissible">
				<span>
				<?php echo esc_html_e( 'Please install newer version of Gutenverse plugin! (v1.1.2 and above)', 'renovater' ); ?>
				</span>
			</div>
			<?php } ?>
	...
	public function theme_setup() {
		load_theme_textdomain( 'renovater', RENOVATER_DIR . '/languages' );
		...
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary', 'renovater' ),
			)
		);
	...
	public function enqueue_scripts() {
		...
		wp_add_inline_style( 'renovater-style', $this->load_font_styles() );

		// enqueue additional core css.
		wp_enqueue_style( 'renovater-core-add', RENOVATER_URI . '/assets/css/core-add.css', array(), RENOVATER_VERSION );

		// enqueue core animation.
		wp_enqueue_script( 'renovater-animate', RENOVATER_URI . '/assets/js/index.js', array(), RENOVATER_VERSION, true );
		wp_enqueue_style( 'renovater-animate', RENOVATER_URI . '/assets/css/animation.css', array(), RENOVATER_VERSION );
	...
	public function backend_scripts() {
		...
		wp_enqueue_style(
			'renovater-notice',
			RENOVATER_URI . '/assets/css/notice.css',
			array(),
			RENOVATER_URI
		);
		...
		if ( $screen->id === 'appearance_page_renovater-dashboard' ) {
			// enqueue css.
			wp_enqueue_style(
				'renovater-dashboard',
				RENOVATER_URI . '/assets/css/dashboard.css',
				array(),
				RENOVATER_VERSION
			);
			...
			wp_enqueue_script(
				'renovater-dashboard',
				RENOVATER_URI . '/assets/js/dashboard.js',
				array( 'wp-api-fetch' ),
				RENOVATER_VERSION,
				true
			);

			wp_localize_script( 'renovater-dashboard', 'GutenverseThemeConfig', $this->theme_config() );
		}
	}
	...
	public function theme_config() {
		return array(
			'demo'    => esc_url( 'https:/gutenverse.com/demo?name=renovater' ),
			'pages'   => array(
				'home'     => RENOVATER_URI . '/assets/img/page-home.webp',
				'about'    => RENOVATER_URI . '/assets/img/page-about.webp',
				'services' => RENOVATER_URI . '/assets/img/page-services.webp',
				'pricing'  => RENOVATER_URI . '/assets/img/page-pricing.webp',
				'faq'      => RENOVATER_URI . '/assets/img/page-faq.webp',
			),
			'domain'  => 'renovater',
			'plugins' => array(
				array(
	...
	public function load_font_styles() {
		$font_families = array(
			...
			'IBM%20Plex%20Sans:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;1,100;1,200;1,300;1,400;1,500;1,600',
		);
